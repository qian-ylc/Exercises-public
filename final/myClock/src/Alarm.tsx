...
const Alarm: React.FC<Props> = (props: Props) => {
    ...
    useEffect(() => {
        if (alarmTime) {
            ...
            if (props.time.getHours() === alarmHour && props.time.getMinutes() === alarmMinute) {
                const dialog: HTMLDialogElement | null = document.querySelector('.alarmRinging');
                if (dialog) {
                    dialog.showModal();
                }
    ...
    }, [props.time.getMinutes(), alarmTime]);
