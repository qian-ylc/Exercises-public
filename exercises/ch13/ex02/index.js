...
function f3() {
    ...
    // 回答:
    // すぐにCが出力され、その後Aが出力され、最後にエラーXが発生した。
...
function f5() {
    ...
    // 回答: 1秒後に B が出力され、その1秒後に A が出力され、その後すぐに 40 が出力される
    // 説明: 2つ目のthenの引数が関数でなくPromiseになっているため、wait1().then(...)が即実行されて、１秒後にBが出力される
    // その後1秒後、wait2()が解決されlogA()が出力されて、その後すぐに40が出力される
    wait2()
        .then(() => {
            logA();
            return 40;
...
function f8() {
    ...
    // 回答: 1秒後に エラーXのメッセージ が出力され、その後すぐに A が出力される
...
function f11() {
    ...
    // 回答: すぐに Xのメッセージ が出力される
    // 説明: new PromiseにエラーXが発生すると、rejectによって捉えて、catchでエラーXが出力される
